...
public class TestClassGenerator implements Constants
{
  ...
  /**
   * This is the boolean representing if the tests needs to be generated for
   * both public and protected methods of the class.
   */
  ...
  {
    my_writer.print("// This class was generated by JMLUNITNG on " + new Date());
    ...
    my_writer.print("import org.jmlspecs.jmlunitng.ShortFormTestListener;");
                                 ...
                                 final JTypeDeclarationType the_decl)
  {
    while (the_method_iter.hasNext())
    {
      ...
      if (obj instanceof JConstructorDeclaration)
      {
        ...
        my_writer.print(BLK_END);
        my_writer.indent(LEVEL2);
        my_writer.print(CATCH + SPACE + "(final" +
                        " org.jmlspecs.jmlrac.runtime.JMLEntryPreconditionError the_exp)");
        my_writer.indent(LEVEL2);
        my_writer.print(BLK_ST);
        my_writer.indent(LEVEL3);
        my_writer.print("// meaningless test");
        my_writer.indent(LEVEL3);
        ...
        my_writer.print(CATCH + SPACE +
                        " (final org.jmlspecs.jmlrac.runtime.JMLAssertionError" +
                        " the_exp)");
        my_writer.indent(LEVEL2);
        my_writer.print(BLK_ST);
        my_writer.indent(LEVEL3);
        my_writer.print("// test failure");
        my_writer.indent(LEVEL3);
        my_writer.print("throw " + "new TestException(the_exp.getMessage());");
        my_writer.indent(LEVEL2);
        my_writer.print(BLK_END);
        my_writer.indent(LEVEL2);
        my_writer.print(CATCH + SPACE +
        ...
        my_writer.print(BLK_ST);
        my_writer.indent(LEVEL3);
        my_writer.print("// test failure for some reason other than assertion violation");
        my_writer.indent(LEVEL3);
        my_writer.print("throw " + "new TestException(the_exp.getMessage());");
        my_writer.indent(LEVEL2);
        my_writer.print(BLK_END);
...
/*        my_writer.indent(LEVEL2);
        ...
        my_writer.print(BLK_END);
        my_writer.indent(LEVEL1);
        my_writer.print(BLK_END); */ /* commented out by dmz, we need these as failures */
  ...
  private void printMethodJavaDoc(final Object the_method, final String the_name)
  {
    ...
    if (the_method instanceof JConstructorDeclaration)
    {
      ...
      {
        my_writer.print("@Test(dataProvider" + " = \"p_" + the_name + "\"" + ")");
      }
    ...
    else if (the_method instanceof JMethodDeclaration)
    {
      final JMethodDeclaration method = (JMethodDeclaration) the_method;
      my_writer.indent(LEVEL1);
      my_writer.print(JDOC_ST + " This method is a test for method " + method.ident() +
                      " from the ");
      my_writer.indent(LEVEL1);
      my_writer.print(" * class to  be tested.");
      my_writer.indent(LEVEL1);
  ...
  private void printMainMethod()
  {
    ...
    my_writer.indent(LEVEL2);
    my_writer.print("final ShortFormTestListener listener = new ShortFormTestListener();");
    my_writer.indent(LEVEL2);
  ...
  {
    final boolean is_static = Utils.hasFlag(the_method.modifiers(), JMethodDeclaration.ACC_STATIC);
    my_writer.indent(LEVEL1);
    my_writer.printOnLine("public void " + the_name + "(");
    
    if (!is_static)
    {
      my_writer.printOnLine("final " + the_decl.ident() + " the_obj");
    }
    ...
    {
      my_writer.indent(LEVEL2);
      my_writer.print("if (the_obj == null)");
      my_writer.indent(LEVEL2);
      my_writer.print(BLK_ST);
      my_writer.indent(LEVEL3);
      my_writer.print("// couldn't construct the object, so this test doesn't count");
      my_writer.indent(LEVEL3);
      my_writer.print("throw new PreconditionSkipException(\"could not construct an object\");");
      my_writer.indent(LEVEL2);
      my_writer.print(BLK_END);
    }
    my_writer.indent(LEVEL2);
    ...
    {
      my_writer.printOnLine(the_decl.ident() + "." + the_method.ident() + BKT_ST);
    }
    for (int i = 0; i < the_method.parameters().length; i++)
    {
    ...
    my_writer.print("// test failure");
    my_writer.indent(LEVEL3);
    my_writer.print("throw " + "new TestException(the_exp);");
    my_writer.indent(LEVEL2);
    my_writer.print(BLK_END);
    my_writer.indent(LEVEL2);
    my_writer.print(CATCH + SPACE +
    ...
    my_writer.print(BLK_END);
    my_writer.indent(LEVEL1);
    my_writer.print(BLK_END);/*    my_writer.indent(LEVEL2);
    my_writer
        .print("catch" +
